...
class _HTaskCardState extends State<HTaskCard> {
  ...
  void initState() {
    super.initState();
    fromCtl = TextEditingController(text: widget.task != null ? widget.task!['from'] : '');
    toCtl = TextEditingController(text: widget.task != null ? widget.task!['to'] : '');
    ...
    if (from.isNotEmpty && to.isNotEmpty && points != 0 && title.isNotEmpty) {
      if (add) {
        ...
        widget.updateTask({
          ...
          'from': from, 'to': to, 'points': points, 'title': title,
          'is_done': false
        });
      }
      return true;
